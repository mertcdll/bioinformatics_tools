...
at = "bwtsw"
out_path = "/home/genwork2/Mert/NIPT/NIPT_BAMS_HG19"
threads = 40
remove_dups = True
remove_seq_dups = False
use_md = False
input_dir = "/home/genwork2/03.Fastq_Output/240902_NDX550927_0503_AHGJT7BDXY"
...
def listFastqs(dir):
    ...
    for r1_file in r1_files:
        ...
        if r2_file in r2_files:
            r1_realpath = os.path.join(dir, r1_file)
            r2_realpath = os.path.join(dir, r2_file)
            sample_id = r2_file.split("_")[0]
            if sample_id not in Fastqs:
                Fastqs[sample_id] = {}
            Fastqs[sample_id]["r1_path"] = r1_realpath
            Fastqs[sample_id]["r2_path"] = r2_realpath
        else:
...
fastq_dict = listFastqs(input_dir)


for sample_id, fastq_paths in fastq_dict.items():

    r1 = fastq_paths["r1_path"]
    r2 = fastq_paths["r2_path"]
