...
class Aligner:
    ...
    def align(self):
        ...
        if self.tool == 'star':
            if self.read2:
                os.system(f"STAR --runThreadN {self.thread_number} --genomeDir {self.index_directory} --readFilesIn {decompressedfile1} {decompressedfile2} --outFileNamePrefix {os.path.join(sample_directory, '')}")    
            else:
                os.system(f"STAR --runThreadN {self.thread_number} --genomeDir {self.index_directory} --readFilesIn {decompressedfile1} --outFileNamePrefix {os.path.join(sample_directory, '')}")    
        elif self.tool == 'hisat2':
            if self.read2:
                os.system(f"hisat2 -p {self.thread_number} -x {os.path.join(self.index_directory, 'genome_ind')} -1 {decompressedfile1} -2 {decompressedfile2} -S {os.path.join(sample_directory, 'Aligned.out.sam')} > {os.path.join(sample_directory, 'initial_metrics.txt')}")
            else:
                os.system(f"hisat2 -p {self.thread_number} -x {os.path.join(self.index_directory, 'genome_ind')} -U {decompressedfile1} -S {os.path.join(sample_directory, 'Aligned.out.sam')} > {os.path.join(sample_directory, 'initial_metrics.txt')}")       
        elif self.tool == 'magicblast':
            if self.read2:
                os.system(f"magicblast -query {decompressedfile1} -query_mate {decompressedfile2} -db {os.path.join(self.index_directory, 'genome_ind')} -num_threads {self.thread_number} -out {os.path.join(sample_directory, 'Aligned.out.sam')} -infmt fastq -outfmt sam")
            else:
                os.system(f"magicblast -query {decompressedfile1} -db {os.path.join(self.index_directory, 'genome_ind')} -num_threads {self.thread_number} -out {os.path.join(sample_directory, 'Aligned.out.sam')} -infmt fastq -outfmt sam")
    ...
    def quantification(self):
        ...
        if self.tool == "featurecounts":
            if self.read_type == "paired-end":
                os.system(f"featureCounts -T {self.thread_number} -t exon -g gene_id -a {self.gtf_file} -o {os.path.join(sample_directory, 'gene_level_counts.txt')} {os.path.join(self.alignment_directory, 'final.bam')} -p --countReadPairs")
            elif self.read_type == "single-end": 
                os.system(f"featureCounts -T {self.thread_number} -t exon -g gene_id -a {self.gtf_file} -o {os.path.join(sample_directory, 'gene_level_counts.txt')} {os.path.join(self.alignment_directory, 'final.bam')}")               
            else:
                raise ValueError("Invalid read_type. Supported vales are 'paired-end' and 'single-end'")
        if self.tool == "htseq":
            if self.read_type == "paired-end":
                os.system(f"htseq-count -f bam -r pos -i gene_id -m union -s no {os.path.join(self.alignment_directory, 'final.bam')} {self.gtf_file} > {os.path.join(sample_directory, 'gene_level_counts.txt')}")
            elif self.read_type == "single-end":
                os.system(f"htseq-count -f bam -r pos -i gene_id -m union -s no {os.path.join(self.alignment_directory, 'final.bam')} {self.gtf_file} > {os.path.join(sample_directory, 'gene_level_counts.txt')}")
            else:
...
alignutil = "magicblast" # change if you want to use a different aligner (other options are star and magicblast)
countutil = "htseq" # change if you want to use a different counter (other option is htseq)
genome_fasta_path = "/home/genwork2/Mert/GRCh38_latest_genomic.fna" # change according to data source
...
thread_num = 40
read_1 = "/home/genwork2/Mert/RNAseq/samplernaseqdata/HBR_Rep1_ERCC-Mix2_Build37-ErccTranscripts-chr22.read1.fastq"
read_2 = "/home/genwork2/Mert/RNAseq/samplernaseqdata/HBR_Rep1_ERCC-Mix2_Build37-ErccTranscripts-chr22.read2.fastq"
...
sample_id = "HBR_Rep1_ERCC-Mix2"

align_dir = "/home/genwork2/Mert/RNAseq/alignment_output/HBR_Rep1_ERCC-Mix2"
...
aligner = Aligner(sample_name=sample_id, read1=read_1, read2=read_2, index_directory=index_dir, output_directory= output_path, thread_number=thread_num, tool=alignutil)
...
# Count aligned reads
counter = Quant(sample_name=sample_id, output_directory=output_path, gtf_file= gtf_path, alignment_directory= align_dir, thread_number=thread_num, tool=countutil, read_type=seqtype)
