...
# Set parameters
input_dir = "/home/genwork2/Mert/NIPT/NIPT_BAMS_HG19"
exome_bait = "/home/genwork2/Mert/02.BEDS/bins_50k.bed"
num_threads = 40
...
def create_count_matrix(input_dir):
    counts_paths = [os.path.join(input_dir, item) for item in os.listdir(input_dir) if item.endswith("_counts.tsv")]
    file_names = [item for item in os.listdir(input_dir) if item.endswith("_counts.tsv")]
    ...
    count_mat.columns = ["chromosome", "start", "end", "bin_count", counts_names[0]]
    ...
    # Save the final count matrix
    count_mat.to_csv(os.path.join(input_dir, "Counts_matrix.txt"), sep="\t", index=False)


create_count_matrix("/home/genwork2/Mert/NIPT/NIPT_BAMS_HG19")