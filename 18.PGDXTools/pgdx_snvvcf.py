...
class CSVtoVCFConverter:
    ...
    """Converts SNV-Indel_EPC-RUO.csv files into ilyome compatible vcf files."""
    ...
    def process_csv_files(self):
        for root, dirs, files in os.walk(self.input_folder):
            for file in files:
                if file.endswith("SNV-Indel_EPC-RUO.csv"):
                    file_path = os.path.join(root, file)
                   
                    df = pd.read_csv(file_path, sep=",")
                    df.replace({np.nan: ''}, inplace=True)
                    df.replace(';', '_', regex= True, inplace=True)
                    df["END"] = df['Nucleotide Position (Genomic hg19)'].str.split(":", expand=True)[1].str.split("-", expand=True)[1]
                    ...
                    vcf_df['POS'] = df['Nucleotide Position (Genomic hg19)'].str.split(":", expand=True)[1].str.split("-", expand=True)[0]
                    vcf_df['ID'] = "."
                    ...
                    vcf_df['REF'] = df['Reference'].apply(lambda x: "N" if x =="" else x)
                    vcf_df['ALT'] = df['Mutant'].apply(lambda x: "." if x =="" else x)
                    vcf_df['QUAL'] = 30
                    ...
                    vcf_columns = ['Nucleotide Position (Genomic hg19)','Reference', 'Mutant', 'Overall Run QC']
