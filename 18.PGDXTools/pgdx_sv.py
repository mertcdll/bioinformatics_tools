...
class CSVtoVCFConverter:
    ...
    """Converts translocations_EPC-RUO.csv files into ilyome compatible vcf files."""
    ...
    def process_csv_files(self):
        for root, dirs, files in os.walk(self.input_folder):
            for file in files:
                if file.endswith("translocations_EPC-RUO.csv"):
                    ...
                    df["SVLEN"] = df.apply(lambda row: int(row["Side 2 Approximate Breakpoint"].split(":")[1]) - int(row["Side 1 Approximate Breakpoint"].split(":")[1]) if row["Intrachromosomal Type"] == "Inversion" else None, axis=1)

                    df["END2"] = df["Side 2 Approximate Breakpoint"].str.split(":", expand=True)[1]

                    df["CHR2"] = df["Side 2 Approximate Breakpoint"].str.split(":", expand=True)[0]

                    df["SVTYPE"] = df.apply(lambda row: "INV" if row["Intrachromosomal Type"] == "Inversion" else "BPD", axis=1)

                    vcf_columns = ['CHROM', 'POS', 'ID', 'REF', 'ALT', 'QUAL', 'FILTER', 'INFO', 'FORMAT', 'PGDX']

                    vcf_df = pd.DataFrame(columns=vcf_columns)

                    vcf_df['CHROM'] = df['Side 1 Approximate Breakpoint'].str.split(":", expand=True)[0]
                    vcf_df['POS'] = df['Side 1 Approximate Breakpoint'].str.split(":", expand=True)[1]
                    vcf_df['ID'] = "."

                    vcf_df['REF'] = "N"
                    vcf_df['ALT'] = df.apply(lambda row: f"<{row['SVTYPE']}>", axis=1)
                    vcf_df['QUAL'] = 30
                    ...
                    vcf_columns = ['Side 1 Approximate Breakpoint', 'Overall Run QC', 'Side 2 Approximate Breakpoint', 'Intrachromosomal Type']
                    ...
                    os.system(f"bcftools norm {vcf_wheader} -N -f {self.ref_fasta} -c s > {final_vcf}")
